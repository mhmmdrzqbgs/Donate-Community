...
if (isset($_POST["id"]) && !empty($_POST["id"])) {
    ...
    $sql_delete = "DELETE FROM tb_jadwal2 WHERE id = ?";
    if ($stmt = mysqli_prepare($link, $sql_delete)) {
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            $sql_alter = "ALTER TABLE tb_jadwal2 AUTO_INCREMENT = 1";
            if (mysqli_query($link, $sql_alter)) {
                // Mendapatkan data ID yang tersisa setelah penghapusan
                $sql_select = "SELECT id FROM tb_jadwal2 ORDER BY id ASC";
                $result = mysqli_query($link, $sql_select);
                if ($result) {
                    $new_id = 1;
                    while ($row = mysqli_fetch_assoc($result)) {
                        $old_id = $row['id'];
                        $sql_update = "UPDATE tb_jadwal2 SET id = ? WHERE id = ?";
