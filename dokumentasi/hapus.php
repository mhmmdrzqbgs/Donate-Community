...
if (isset($_POST["id"]) && !empty($_POST["id"])) {
    ...
    $sql_delete = "DELETE FROM tb_dok WHERE id = ?";
    if ($stmt = mysqli_prepare($link, $sql_delete)) {
        mysqli_stmt_bind_param($stmt, "i", $id);
        if (mysqli_stmt_execute($stmt)) {
            mysqli_stmt_close($stmt);

            $sql_select = "SELECT id FROM tb_dok ORDER BY id ASC";
            $result = mysqli_query($link, $sql_select);
            if ($result) {
                $new_id = 1;
                while ($row = mysqli_fetch_assoc($result)) {
                    $old_id = $row['id'];
                    $sql_update = "UPDATE tb_dok SET id = ? WHERE id = ?";
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="page-header">
                        <h1>Hapus Dokmentasi</h1>
                    </div>
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                        <div class="alert alert-danger fade in">
                            <input type="hidden" name="id" value="<?php echo trim($_GET["id"]); ?>"/>
                            <p>Apakah anda yakin untuk menghapus catatan ini?</p><br>
                            <p>
